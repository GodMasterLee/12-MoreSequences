...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(180.0, 100.0),
                rg.Point(159.3, 128.5),
                rg.Point(125.7, 117.6),
                rg.Point(125.7, 82.4),
                rg.Point(159.3, 71.5)]  # All numbers are approximate.
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  6 red dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    draw_points_on_circle(window, circle, 6, 'red')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  6 black slices on yellow circle, thin (thickness=2) blue lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 2
    circle.fill_color = 'yellow'
    pizza(window, circle, 6, 'black', 2)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(generate_points_on_circle(circle, number_of_slices))):
        line = rg.Line(generate_points_on_circle(circle, number_of_slices)[k],circle.center)
        line.color = color
...
def run_test_polygon():
    ...
    # Tests 3:
    title = 'POLYGON tests 3:  8 segments with thin black lines.'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(150, 150), 100)
    circle.outline_thickness = 3
    polygon(window, circle, 8, 'black', 3)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(generate_points_on_circle(circle,number_of_segments))-1):
        line = rg.Line(generate_points_on_circle(circle,number_of_segments)[k],generate_points_on_circle(circle,number_of_segments)[k+1])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
        window.render()

    line = rg.Line(generate_points_on_circle(circle,number_of_segments)[len(generate_points_on_circle(circle,number_of_segments))-1],generate_points_on_circle(circle,number_of_segments)[0])
    line.color = color
    line.thickness = thickness
    line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  20 lime green lines on blue circle, hops = 5.'
    window = rg.RoseWindow(480, 350, title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    line = rg.Line(generate_points_on_circle(circle, number_of_lines)[
                       len(generate_points_on_circle(circle, number_of_lines)) - 1],
                   generate_points_on_circle(circle, number_of_lines)[0])
    line.color = color
    line.thickness = thickness
    line.attach_to(window)
    window.render()
